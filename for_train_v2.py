      ...
      for i in range(len(ms)):
          ...
          if idx_25min != 0 and idx_fwd_5min != 0:
              train_data.append((
                  ...
                  (buy_vol[i]+1) / (sell_vol[i]+1), 
                  (buy_vol[idx_5min]+1) / (sell_vol[idx_5min]+1),
                  (buy_vol[idx_10min]+1) / (sell_vol[idx_10min]+1),
                  stdev(price[idx_5min:i]),
                  stdev(price[idx_10min:i]),
                  stdev(price[idx_15min:i]),
                  stdev(price[idx_25min:i]),
...
train_data = pd.DataFrame(train_data, columns=['y'] + [f'x{i}' for i in range(1, 10)])
